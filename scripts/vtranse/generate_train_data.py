...
def get_rel_mat(eid2idx, scene_graph, ent_dict:SymbolDictionary, pred_dict: SymbolDictionary):
    ...
    for sbj_id, entity in scene_graph["objects"].items():
        if sbj_id not in eid2idx: continue
        relations = entity["relations"]
        for relation in relations:
            if relation["name"] not in pred_dict: continue
            obj_id = relation["object"]
            if obj_id not in eid2idx: continue
...
def get_roidb_from_proposals(proposals, gt_boxes, ent_labels, rel_mat, threshold=0.5, pred_use_prob=None):
...
def get_roidb_from_gt(gt_boxes, ent_labels, rel_mat, pred_use_prob=None):
...
if __name__ == "__main__":
    ...
    pred_use_prob = json.load(open(pred_use_prob_path)) if pred_use_prob_path is not None else None
    random.seed(999)
    n_ent = len(ent_dict)
    n_pred = len(pred_dict)

    out_path = "data/gqa/vrd/vtranse/input/gqa_%s_%d_%d_roidb.npz" % (box_source, n_ent, n_pred)
    ...
    for roidb_key, sg_file in tqdm(zip(roidb_keys, scene_graph_files)):
        ...
        for image_id, scene_graph in tqdm(scene_graphs.items()):
            ...
            rel_mat = get_rel_mat(eid2idx, scene_graph, ent_dict, pred_dict)

            if box_source == "gt":
                sbj_boxes, obj_boxes, rlp_labels = get_roidb_from_gt(ent_boxes, ent_labels, rel_mat, pred_use_prob)
            elif box_source == "proposal":
                proposals = h5_boxes[meta["file"]][meta["idx"], :meta["objectsNum"], :]
                sbj_boxes, obj_boxes, rlp_labels = get_roidb_from_proposals(proposals, ent_boxes, ent_labels, rel_mat, pred_use_prob)
            else:
